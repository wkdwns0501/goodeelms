					...
					<div class="card-body">
								...
								<table class="table table-bordered info-table">
									<tbody>
										...
										<tr>
											...
											<td class="text-start" colspan="3">${student.studentAddress}</td>
										</tr>
										<tr>
										...
										<tr>
											...
											<td class="text-start" colspan="3"><c:set var="bankParts"
													value="${fn:split(student.studentBank, ' ')}" />
												${bankParts[0]} / ${bankParts[1]}</td>
										</tr>
									</tbody>
								</table>
							</div>
						...
						<div class="d-flex justify-content-end gap-2 mt-3">
							...
							<button class="btn btn-outline-primary" type="button"
								data-bs-toggle="collapse" data-bs-target="#pwFormWrap">
								비밀번호 변경하기</button>
						</div>
						<div id="mypageForms">
							<!-- 인적 사항 수정 폼 -->
							<div class="collapse mt-4" id="editForm"
								data-bs-parent="#mypageForms">
								<div class="card card-body border">
									...
									<form method="post" id="profileForm"
										...
										<input type="hidden" name="nowPhotoFile"
											value="<c:out value='${student.photoFile}'/>"> <input type="hidden"
											name="nowPhotoUUID" value="<c:out value='${student.photoUUID}'/>">
										<div class="row g-3">
												...
												<div class="row mb-3">
													...
													<div class="col-md-6">
														<label class="form-label">전화번호</label> <input
															type="hidden" name="studentPhone" id="studentPhoneHidden"
															value="<c:out value='${student.studentPhone}'/>"/>

														<div class="d-flex align-items-center gap-2">
															<select class="form-select" style="max-width: 120px;"
																id="phone1">
																<option value="<c:out value='010'/>" ${phone1=='010' ? 'selected' : ''}>010</option>
																<option value="<c:out value='011'/>" ${phone1=='011' ? 'selected' : ''}>011</option>
																<option value="<c:out value='016'/>" ${phone1=='016' ? 'selected' : ''}>016</option>
																<option value="<c:out value='017'/>" ${phone1=='017' ? 'selected' : ''}>017</option>
																<option value="<c:out value='018'/>" ${phone1=='018' ? 'selected' : ''}>018</option>
																<option value="<c:out value='019'/>" ${phone1=='019' ? 'selected' : ''}>019</option>
															</select> <span class="text-muted">-</span> <input type="text"
												       class="form-control" style="max-width: 140px;"
												       id="phone2" maxlength="4"
												       value="<c:out value='${phone2}'/>"
												       placeholder="1234"> <span class="text-muted">-</span>

															<input type="text"
												       class="form-control" style="max-width: 140px;"
												       id="phone3" maxlength="4"
												       value="<c:out value='${phone3}'/>"
												       placeholder="5678">
														</div>
														<div id="phoneError" class="invalid-feedback d-none">
															전화번호는 가운데/끝 4자리 숫자로 입력해주세요.</div>
													</div>

													<!-- 이메일 -->
													<div class="col-md-6">
														<label class="form-label">이메일</label> <input type="hidden"
												       name="studentEmail" id="studentEmailHidden"
												       value="<c:out value='${student.studentEmail}'/>" />


														<div class="d-flex align-items-center gap-2">
															<input type="text" class="form-control" id="emailId"
															       value="<c:out value='${emailId}'/>" placeholder="아이디">
												...
												<div class="row mb-3">
													<div class="col-md-5">
														...
														<select name="bank_name" class="form-select">
														  <option value="<c:out value='국민은행'/>" ${bankName=='국민은행' ? 'selected' : ''}>국민은행</option>
														  <option value="<c:out value='농협은행'/>" ${bankName=='농협은행' ? 'selected' : ''}>농협은행</option>
														  <option value="<c:out value='신한은행'/>" ${bankName=='신한은행' ? 'selected' : ''}>신한은행</option>
														  <option value="<c:out value='우리은행'/>" ${bankName=='우리은행' ? 'selected' : ''}>우리은행</option>
														  <option value="<c:out value='카카오뱅크'/>" ${bankName=='카카오뱅크' ? 'selected' : ''}>카카오뱅크</option>
														</select>
													</div>
													<div class="col-md-7">
														...
														<input type="text"
														       ...
														       value="<c:out value='${accountNumber}'/>"
														       class="form-control"
														       placeholder="숫자와 - 만 입력">
														<div id="accountError" class="invalid-feedback d-none">
															계좌번호는 000-0000000 형식(숫자3자리-숫자7자리)으로 입력해주세요.</div>
													</div>
												</div>
												<!-- 현재 비밀번호 확인 -->
												<div class="mb-3">
													<label for="confirmPassword" class="form-label">현재
							...
							<div class="collapse mt-3" id="pwFormWrap"
								...
								<div class="card card-body border">
									...
									<form id="pwForm" method="post"
										action="<c:url value='/student/mypage/password'/>" novalidate>
										...
										<div class="row g-3">
											<div class="col-md-6">
												...
												<div id="currentPwError" class="invalid-feedback d-none">현재
													비밀번호를 입력해주세요.</div>
											</div>
											<div class="col-md-6">
