    ...
    <hr class="my-2 mx-3" /> 
  	<c:choose>
	  	...
	  	<c:when test="${sessionScope.user_role == 'PROFESSOR'}">
			  <a class="nav-link" href="<c:url value='/professor/lecture'/>">
...
<script>
  (function () {
    const currentPath = window.location.pathname; // 예: /professor/lecture/list
    const navLinks = document.querySelectorAll('.sidebar .nav-link');

    navLinks.forEach(link => {
      // link.href는 절대경로가 될 수 있으니 pathname만 뽑아서 비교
      const linkPath = new URL(link.href, window.location.origin).pathname;

      // 현재 경로가 메뉴 경로로 시작하면 active
