...
public class StudentController extends HttpServlet {
	...
	private void signup(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    String str = request.getParameter("student_id");
	    ...
	    if(studentService.checkColumnByDTO(updateStudent)) {
	        request.setAttribute("errorMessage", "이메일이나 전화번호가 이미 사용중입니다.");
	        request.setAttribute("studentId", studentId);
	        request.getRequestDispatcher("/WEB-INF/views/student/studentSignUp.jsp").forward(request, response);
	        return;
	    }
	    ...
	    if (isUpdated) {
	        ...
	        response.sendRedirect(request.getContextPath() + "/main.jsp?success=true");
	        return;
	    } else {
	        request.setAttribute("errorMessage", "수정 실패. 입력 데이터를 확인해주세요.");
	        request.setAttribute("studentId", studentId);
	        request.getRequestDispatcher("/WEB-INF/views/student/studentSignUp.jsp").forward(request, response);
	        return;
	    }
	...
	protected void payTuition(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		HttpSession session = request.getSession();
		int student_id = (Integer) session.getAttribute("student_id");
		...
		if (paymentTuition > 0) {
			tService.getTuitionPaymentAfterPay(student_id, paymentTuition);
		}
	...
	protected void showStatusAndMajorHistory(HttpServletRequest request, HttpServletResponse response)
		...
		List<StudentStatusHistoryDTO> statusList = new StudentStatusService().getStatusHistory(student_id); // status_type,
																											// status_reason,
																											// status_at
		...
		if (dto != null)
			request.setAttribute("majorHistory", dto);
		if (majorList == null)
			request.setAttribute("msg", "전공을 조회할 수 없습니다.");
	...
	protected void showGrade(HttpServletRequest request, HttpServletResponse response)
		...
		request.setAttribute("grade", grade); // lecture_code, lecture_name, lecuture_year, lecture_semester,
												// lecture_score ( lecture_history)
	...
	protected void showReawrdAndPunishments(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
	...
	// 학생 강의 목록
	private void showLectureList(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		...
		request.setAttribute("lectures", lectureService.getLecturePageForStudent(page, limit, keyword));
